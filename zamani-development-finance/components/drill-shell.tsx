    ...
    <div style={{ minHeight: "100vh", background: "#F4F6F9" }}>
      <header className="command-bar" style={{ position: "sticky", top: 0, zIndex: 50, padding: "0 28px", height: 56, display: "flex", alignItems: "center", gap: 12 }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <div style={{ width: 28, height: 28, background: "#0D7A42", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", background: "linear-gradient(135deg,#0A3820 0%,#0D7A42 55%,#17A863 100%)", padding: "26px 32px 44px" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
          <nav style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
            {crumbs.map((c, i) => (
              <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                {c.href ? (
                  <Link href={c.href} style={{ fontSize: 12, color: "rgba(255,255,255,0.85)", fontFamily: "var(--font-sans)", textDecoration: "none", fontWeight: 500 }}>
                    {c.label}
                  </Link>
                ) : (
                  <span style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", fontFamily: "var(--font-sans)" }}>{c.label}</span>
                )}
              </span>
            ))}
          </nav>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              {subtitle && <p style={{ fontSize: 14.5, color: "rgba(255,255,255,0.85)", marginTop: 9, fontFamily: "var(--font-sans)", maxWidth: 640, lineHeight: 1.5 }}>{subtitle}</p>}
            </div>
            <button
              disabled
