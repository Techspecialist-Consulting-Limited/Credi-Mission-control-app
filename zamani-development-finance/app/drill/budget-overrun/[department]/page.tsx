      ...
      <div className="enterprise-card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        {records.map((r) => (
          <div key={r.budgetId} style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr 1fr", gap: 12, padding: "13px 24px", alignItems: "center", borderBottom: "1px solid rgba(10,14,26,0.055)" }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.category}</div>
              <div style={{ fontSize: 11, color: "#5A6880", fontFamily: "var(--font-sans)" }}>{r.fiscalPeriod} · {r.budgetId}</div>
            </div>
            <span style={{ fontSize: 13, color: "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.budgeted}</span>
            <span style={{ fontSize: 13, color: "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.committed}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: r.overBudget ? "#DC2626" : "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.actual}</span>
            <span style={{ fontSize: 12, color: r.overBudget ? "#DC2626" : "#059669", fontFamily: "var(--font-sans)" }}>{r.utilisationPct.toFixed(0)}%</span>
          </div>
        ))}
        {records.length === 0 && <div style={{ padding: 24, fontSize: 13, color: "#5A6880", fontFamily: "var(--font-sans)" }}>No budget lines recorded for this department.</div>}
      </div>
    </DrillShell>
  );
}
