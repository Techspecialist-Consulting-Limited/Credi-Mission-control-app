        ...
        {records.map((r) => (
          <div key={r.requisitionId} style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr 1fr 1fr", gap: 12, padding: "13px 24px", alignItems: "center", borderBottom: "1px solid rgba(10,14,26,0.055)" }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: "#5A6880", fontFamily: "var(--font-sans)" }}>{r.requisitionId}</div>
            </div>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.estimatedValue}</span>
            <span style={{ fontSize: 13, color: "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.raisedDate}</span>
            <span style={{ fontSize: 13, color: r.daysOpen > 60 ? "#DC2626" : "#0A0E1A", fontFamily: "var(--font-sans)" }}>{r.daysOpen}d</span>
            <span style={{ fontSize: 13, color: r.bidCount <= 1 ? "#D97706" : "#059669", fontFamily: "var(--font-sans)" }}>{r.bidCount}</span>
          </div>
        ))}
        {records.length === 0 && <div style={{ padding: 24, fontSize: 13, color: "#5A6880", fontFamily: "var(--font-sans)" }}>No open requisitions for this department.</div>}
      </div>
    </DrillShell>
  );
}
