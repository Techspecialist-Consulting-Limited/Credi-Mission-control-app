        ...
        {groups.map((g) => (
          ...
          >
            ...
            <div style={{ height: 8, background: "#EEF2F7", borderRadius: 4, overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${(g.rawTotal / max) * 100}%`, background: "#0D7A42", borderRadius: 4 }} />
            </div>
            <span className="font-display" style={{ fontSize: 14, fontWeight: 700, color: "#0A0E1A", textAlign: "right" }}>{g.totalValue}</span>
            <span style={{ color: "#0D7A42", fontSize: 12, fontFamily: "var(--font-sans)" }}>View requisitions →</span>
          </Link>
        ))}
      </div>
    </DrillShell>
