      ...
      <div className="enterprise-card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        {records.map((r) => (
          ...
          >
            ...
            <span className="font-mono" style={{ fontSize: 12, color: "#6B7A94" }}>{r.disbursementId}</span>
            <span className="font-display" style={{ fontSize: 13, fontWeight: 700, color: "#B45309" }}>{r.amountOutstanding}</span>
            <span style={{ fontSize: 12, color: "#6B7A94", fontFamily: "Inter" }}>{r.dueDate}</span>
            <span style={{ fontSize: 12, color: "#6B7A94", fontFamily: "Inter" }}>{r.product} · {r.state}</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: r.status === "Defaulted" ? "#DC2626" : "#D97706", fontFamily: "Inter" }}>
              {r.daysOverdue} days{r.status === "Defaulted" ? " · Defaulted" : ""}
            </span>
          </div>
        ))}
        {records.length === 0 && <div style={{ padding: 24, fontSize: 13, color: "#6B7A94", fontFamily: "Inter" }}>No arrears records for this partner.</div>}
      </div>
    </DrillShell>
  );
}
