      ...
      <div className="enterprise-card" style={{ padding: 0, overflow: "hidden" }}>
        ...
        {records.map((r) => (
          ...
          >
            ...
            <span style={{ fontSize: 12, color: "#5A6880", fontFamily: "var(--font-sans)" }}>{r.approvalDate}</span>
            <span style={{ fontSize: 12, color: "#5A6880", fontFamily: "var(--font-sans)" }}>{r.approvedBy}</span>
            <span style={{ fontSize: 12, color: "#5A6880", fontFamily: "var(--font-sans)" }}>{r.product} · {r.state}</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#DC2626", fontFamily: "var(--font-sans)" }}>{r.daysLapsed} days</span>
          </div>
        ))}
        {records.length === 0 && <div style={{ padding: 24, fontSize: 13, color: "#5A6880", fontFamily: "var(--font-sans)" }}>No lapsed approvals for this partner.</div>}
      </div>
    </DrillShell>
  );
}
